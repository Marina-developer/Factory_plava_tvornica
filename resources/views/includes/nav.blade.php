      ...
      <ul class="nav navbar-nav nav-flex-icons ml-auto">
        <li class="nav-item dropdown">
          ...
          <div class="dropdown-menu" aria-labelledby="navbarDropdown">
              @if (app()->isLocale('hr'))
            <div id="change"><a class="dropdown-item" href="{{ route(\Illuminate\Support\Facades\Route::currentRouteName() , ['locale' => 'en' ]) }}">{{ __('factory.english') }}</a></div>
      @elseif(app()->isLocale('en'))
           <div id="change"> <a class="dropdown-item"  href="{{ route(\Illuminate\Support\Facades\Route::currentRouteName(),  ['locale' => 'hr' ]) }}"> {{ __('factory.croatian') }}</a></div>
              @endif
          </div>
